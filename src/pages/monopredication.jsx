...
export function Monopredication()
{
   ...
   const {userstore}=useContext(userContexte)
    ...
    const likerPredication=(id)=>{
       ...
       // copy clipboard 
        ...
        const {idpredication}=useParams('idpredication')

          const {data:predication,isLoading}=useQuery({
            queryKey:['predication','idpredication'],
            queryFn:()=>axios.get(`${process.env.REACT_APP_BACKEND_URL}/predication/${idpredication}`,{
              headers:{
                "Authorization": "Bearer "+token
              }
            })
...
console.log(predication)
   ...
   <div className="flex flex-col justify-center items-center w-full">
    ...
    <div style={{top:'80px'}} className='position-relative flex flex-row justify-between items-start w-full deuxiemedivnavbar'>
      ...
      <p className="text-center text-xl text-violet-700 px-2 rounded-lg mt-3">{predication?.data?.predication?.theme?.toUpperCase()} <br/>  <soan className="text-black">{predication?.data?.predication?.predicateur}</soan></p>
      <div className="h-full flex flex-col justify-center flex-wrap items-center gap-4 pb-24 mt-12 w-full">
      
               {
                predication ?   <div class="position-relative -top-8 bg-white" style={{width:'100%'}}>
                <video autoPlay={true} controlsList='nodownload' src={process.env.REACT_APP_BACKEND_FILE+predication?.data?.predication?.lienvideo} class="card-img-top" controls></video>
                
                <div class="card-body bg-white">
                  <h5 class="card-title font-bold">{predication?.data?.predication?.theme}</h5>
                  <p class="card-text position-relative -top-2"> {predication?.data?.predication?.predicateur}</p>
                  <div className="flex w-full flex-row justify-between">
                  <span className="flex flex-row justify-center items-center gap-x-2">
                  <img id={predication?.data?.predication?.id+'iconcoeursuggestion'} onClick={()=>{likerPredication(predication?.data?.predication?.id)}} className='amenpub position-relative cursor-pointer' style={{height:'20px',transition:'all ease 0.6s'}} src={userstore && predication?.data?.predication?.likeur?.includes(userstore?.data?.id+',') ? coeurv : coeur} alt="" /> <span id={'nombrelike'+e.id}>{predication?.data?.predication?.like}</span>
                  </span>
                  <button onClick={()=>{setId(predication?.data?.predication?.id)}} data-bs-toggle="modal" data-bs-target="#exampleModalpartage" className='flex flex-row justify-center items-center gap-x-2 text-sm p-1 px-2 rounded-xl partagepub'><img className='partagepub' style={{height:'19px'}} src={partager} alt="" />{predication?.data?.predication?.partage} </button>
                  </div>
                </div>
