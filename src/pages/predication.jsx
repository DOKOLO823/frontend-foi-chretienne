...
import predic from '../images/predic.jpg';
  ...
  const {token}=useContext(tokenContexte)
    ...
    const likerPredication=(id)=>{
        if(document.getElementById(id+'iconcoeursuggestion').src==coeur){
          ...
          document.getElementById(id+'iconcoeursuggestion').style.transform='scale(1.2)'
           ...
           token ?  axios.post(`${process.env.REACT_APP_BACKEND_URL}/add-like-predication`,{id},{
            headers:{
       ...
       // copy clipboard 
         ...
         const [predicationsstate, setPredicationsstate]=useState([])
         const [loading, setLoading]=useState(false)

          const {data:predications,isLoading}=useQuery({
            queryKey:['predications'],
            queryFn:()=>axios.get(`${process.env.REACT_APP_BACKEND_URL}/predications`,{
              headers:{
                "Authorization": "Bearer "+token
              }
            })
           ...
           useEffect(()=>{
            setPredicationsstate(predications?.data?.predications)
           },[predications])
          ...
          const partagewhatsapp=()=>{
            setLoadpartage(true)
            axios.post(`${process.env.REACT_APP_BACKEND_URL}/add-partage-predication`,{id},{
              headers:{
                "Authorization": "Bearer "+token
              }
            }).then((res)=>{
              setLoadpartage(false)
              window.location.href="https://wa.me/?text=https://foichretienne.org/predication/"+id
            })
          }
          ...
          const partagefacebook=()=>{
            ...
            axios.post(`${process.env.REACT_APP_BACKEND_URL}/add-partage-predication`,{id},{
              ...
              setLoadpartage(false)
              window.location.href="https://www.facebook.com/sharer/sharer.php?u=https://foichretienne.org/predication/"+id+";src=sdkpreparse"
            })
          }
          ...
          useEffect(()=>{
            if(recherche==null || typeof(recherche)==undefined){
             setPredicationsstate(predications?.data?.predications)
              ...
              const filter=()=>predications?.data?.predications?.filter(el=>((el?.theme?.toLowerCase()?.includes(recherche?.trim()?.toLowerCase())) || (el?.predicateur?.toLowerCase()?.includes(recherche?.trim()?.toLowerCase()))))
              setPredicationsstate(filter)
            }
          },[recherche])
   ...
   <div className="flex flex-col justify-center items-center w-full">
    ...
    <div style={{top:'80px'}} className='position-relative flex flex-row justify-between items-start w-full deuxiemedivnavbar'>
      ...
      <div className='text-center bg-white p-4 font-italic w-full rounded-lg position-relative -top-2 psaume'>Il leur dit : &lt;&lt;Allez dans le monde entier. Proclamez l'Évangile à toute la création. Celui qui croira et sera baptisé sera sauvé ; celui qui refusera de croire sera condamné.<br/><br/><span className="bg-violet-700 text-white p-1 rounded-lg px-3 font-bold">Marc 16 : 15-16</span></div>
      <p className="text-center text-xl text-violet-700 px-2 rounded-lg mt-3">Prédications :</p>
      ...
      <div className="h-full flex flex-col justify-center flex-wrap items-center gap-4 pb-24 mt-12">
        {
            predications ? predicationsstate?.map((e,i)=>{
                return <div class="position-relative -top-8 bg-white" style={{width:'100%'}}>
                <img src={predic} class="card-img-top"/>
                
                <div class="card-body bg-white">
                  <h5 class="card-title font-bold">{e.theme}</h5>
                  <p class="card-text position-relative -top-2"> {e.predicateur}</p>
                  ...
                  <Link to={'/predication/'+e.id} className="w-full btn bg-violet-700 text-white text-center focus:bg-violet-600 active:bg-violet-600 hover:bg-violet-600 mt-2">Suivre</Link>
                </div>
...
<div style={{width:'100%',flexWrap:'wrap'}} class=" gap-2 w-full flex flex-row">
    <CopyToClipboard text={'https://foichretienne.org/predication/'+id}
