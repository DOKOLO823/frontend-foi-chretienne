...
export default function NotifierUser() {
    ...
    const send=()=>{
      ...
      token ?  axios.post(`${process.env.REACT_APP_BACKEND_URL}/notifier-users`,{info},{
          headers:{
    ...
    return (
      <>
       ...
       <span onClick={()=>{navigate(-1)}} className='cursor-pointer'><img className='float-left' style={{height:'16px'}} src={left} alt="retour premiers amis" /></span>
         ...
         Envoyer des notifications aux users de FOI CHRETIENNE
            </h2>
          </div>
            ...
            <div className="space-y-6 w-full">
              <div>
                ...
                <div className="mt-2 w-full">
                  <textarea onChange={(e)=>{setInfo(e.target.value)}} className='w-full rounded-lg focus:border-violet-700 focus:border-2 border-solid' rows={6} placeholder='Entrez les details' name="" id=""></textarea>
                </div>
              </div>
              ...
              <div>
                <button
                onClick={send}
                disabled={info?.trim()?.length>0 ? false : true}
                  type="submit"
